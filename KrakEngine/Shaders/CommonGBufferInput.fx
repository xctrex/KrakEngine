...
Texture2D<float4> PositionLuminanceTexture      : register(t3);
...
SurfaceData UnpackGBuffer(float2 UV)
{
    ...
    // Get the position
    const float4 positionLuminance = PositionLuminanceTexture.Sample(PointSampler, UV.xy);
    output.Position = positionLuminance.xyz;
    output.Luminance = positionLuminance.w;
    return output;
}
